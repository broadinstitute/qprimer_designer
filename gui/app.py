...
VIRUS_PRESETS: list[tuple[str, int]] = [
    ...
    ("Rubella virus", 11041),
    # --- Hemorrhagic fever ---
    ("Orthoebolavirus (all Ebola)", 3044781),
...
def _page_virus_map():
    ...
    with col_left:
        ...
        if has_records:
            # Parse dates
            rec_df["date_str"] = rec_df["release_date"].where(
                rec_df["release_date"] != "", rec_df["collection_date"],
            )
...
# ---------------------------------------------------------------------------
# Delphy phylogenetic tree helpers — DISABLED (needs careful redesign)
# ---------------------------------------------------------------------------
...
# --- Delphy helper functions commented out ---
...
#         return {"error": f"Delphy exited with code {ret}:\n{err_msg}"}
#
#     return {
#         "log": log_file,
#         "trees": trees_file,
...
def _tab_results():
    ...
    # Resolve run directory (used by both design CSV display and Delphy tree)
    ...
    # --- Phylogenetic Tree (Delphy) --- DISABLED
    # if workflow in ("design", "evaluate"):
    # st.divider()
    # st.subheader("Phylogenetic Tree")
#
    # # Check prerequisites: MSA or FASTA file and metadata
    # MSA_DIR = PROJECT_ROOT / "target_seqs" / "msa"
    # target_names = st.session_state.get("targets", [])
    # # Fallback: discover targets from MSA directory, then FASTA directory
    # if not target_names:
    # found = set()
    # if MSA_DIR.exists():
    # found.update(
    ...
    # if FASTA_DIR.exists():
    # found.update(
    # p.stem for p in FASTA_DIR.glob("*.fa")
    # if not p.stem.endswith("_delphy")
    # )
    # target_names = sorted(found)
#
    # if not target_names:
...
#
    ...
    # fasta_path = FASTA_DIR / f"{tree_target}.fa"
    # meta_path = FASTA_DIR / f"{tree_target}_metadata.csv"
    # tree_dir = run_dir / "trees" if run_dir else None
#
    # # If MSA doesn't exist but raw FASTA does, align with MAFFT
    # if not msa_path.exists() and fasta_path.exists():
    # import shutil
    # if shutil.which("mafft") is None:
    # st.warning("MAFFT not found. Install it to generate alignments for Delphy.")
    # else:
    # if st.button(f"Align `{tree_target}` with MAFFT", key="mafft_align"):
    # MSA_DIR.mkdir(parents=True, exist_ok=True)
    # with st.spinner("Running MAFFT alignment... This may take a while."):
    # import subprocess
    # result = subprocess.run(
    # ["mafft", "--auto", "--thread", str(min(8, os.cpu_count() or 1)), str(fasta_path)],
    # capture_output=True, text=True,
    # )
    # if result.returncode == 0:
    # msa_path.write_text(result.stdout)
    # st.success(f"Alignment complete: `{msa_path.name}`")
    # st.rerun()
    # else:
    # st.error(f"MAFFT failed:\n{result.stderr[:500]}")
#
    # if not msa_path.exists() and not fasta_path.exists():
    # st.warning(f"Sequence file not found for `{tree_target}`. Run the pipeline first.")
    # else:
...
#
    ...
    # # _run_delphy strips all "_delphy" from the fasta stem, so match that
    # _tree_stem = tree_target.replace("_delphy", "")
    # dphy_file = tree_dir / f"{_tree_stem}_delphy.dphy" if tree_dir else None
    # trees_file = tree_dir / f"{_tree_stem}_delphy.trees" if tree_dir else None
...
#
    ...
    # st.error(f"Delphy inference failed.\n{err}")
    # else:
...
#
    # if eval_re_files:
    # import pandas as pd
    # # Get primer pairs from final CSV (design) or eval.re (evaluate)
    # if csvs:
    # csv_path = run_dir / csv_labels[0]
    # final_df = pd.read_csv(csv_path)
    # else:
    # final_df = pd.read_csv(eval_re_files[0], usecols=["pname_f", "pname_r"])
    # if len(final_df) > 0:
    # pair_options = [
    # f"{r['pname_f']} / {r['pname_r']}"
    # for _, r in final_df.drop_duplicates(subset=["pname_f", "pname_r"]).iterrows()
